...
$insetClasses = Laraliveui::applyInset($inset, top: '-mt-1', right: '-me-2', bottom: '-mb-1', left: '-ms-2');
...
$classes = Laraliveui::classes()
    ...
    ->add(match ($size) {
        'lg' => 'text-sm py-1.5 **:data-laraliveui-badge-icon:me-2',
        default => 'text-sm py-1 **:data-laraliveui-badge-icon:me-1.5',
        'sm' => 'text-xs py-1 **:data-laraliveui-badge-icon:me-1',
    })
    ->add(match ($rounded) {
...
<laraliveui:button-or-div :attributes="$attributes->class($classes)" data-laraliveui-badge>
    <?php if (is_string($icon) && $icon !== ''): ?>
        <laraliveui:icon :$icon :variant="$iconVariant" :class="$iconClasses" data-laraliveui-badge-icon />
    <?php else: ?>
        {{ $icon }}
    <?php endif; ?>
    ...
    <?php if ($iconTrailing): ?>
        <div class="ps-1 flex items-center" data-laraliveui-badge-icon:trailing>
