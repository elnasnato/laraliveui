...
$iconTrailingClasses = Laraliveui::classes()
...
if ($loading && $type !== 'submit' && ! $isJsMethod) {
    $attributes = $attributes->merge(['wire:loading.attr' => 'data-laraliveui-loading']);
...
$classes = Laraliveui::classes()
    ...
    ->add($inset ? match ($size) { // Inset...
        'base' => $square
            ? Laraliveui::applyInset($inset, top: '-mt-2.5', right: '-me-2.5', bottom: '-mb-2.5', left: '-ms-2.5')
            : Laraliveui::applyInset($inset, top: '-mt-2.5', right: '-me-4', bottom: '-mb-3', left: '-ms-4'),
        'sm' => $square
            ? Laraliveui::applyInset($inset, top: '-mt-1.5', right: '-me-1.5', bottom: '-mb-1.5', left: '-ms-1.5')
            : Laraliveui::applyInset($inset, top: '-mt-1.5', right: '-me-3', bottom: '-mb-1.5', left: '-ms-3'),
        'xs' => $square
            ? Laraliveui::applyInset($inset, top: '-mt-1', right: '-me-1', bottom: '-mb-1', left: '-ms-1')
            : Laraliveui::applyInset($inset, top: '-mt-1', right: '-me-2', bottom: '-mb-1', left: '-ms-2'),
    } : '')
    ...
    ->add(match ($variant) { // Grouped border treatments...
        'ghost' => '',
        'subtle' => '',
        'outline' => '[[data-laraliveui-button-group]_&]:border-s-0 [:is([data-laraliveui-button-group]>&:first-child,_[data-laraliveui-button-group]_:first-child>&)]:border-s-[1px]',
        'filled' => '[[data-laraliveui-button-group]_&]:border-e [:is([data-laraliveui-button-group]>&:last-child,_[data-laraliveui-button-group]_:last-child>&)]:border-e-0 [[data-laraliveui-button-group]_&]:border-zinc-200/80 dark:[[data-laraliveui-button-group]_&]:border-zinc-900/50',
        'danger' => '[[data-laraliveui-button-group]_&]:border-e [:is([data-laraliveui-button-group]>&:last-child,_[data-laraliveui-button-group]_:last-child>&)]:border-e-0 [[data-laraliveui-button-group]_&]:border-red-600 dark:[[data-laraliveui-button-group]_&]:border-red-900/25',
        'primary' => '[[data-laraliveui-button-group]_&]:border-e-0 [:is([data-laraliveui-button-group]>&:last-child,_[data-laraliveui-button-group]_:last-child>&)]:border-e-[1px] dark:[:is([data-laraliveui-button-group]>&:last-child,_[data-laraliveui-button-group]_:last-child>&)]:border-e-0 dark:[:is([data-laraliveui-button-group]>&:last-child,_[data-laraliveui-button-group]_:last-child>&)]:border-s-[1px] [:is([data-laraliveui-button-group]>&:not(:first-child),_[data-laraliveui-button-group]_:not(:first-child)>&)]:border-s-[color-mix(in_srgb,var(--color-accent-foreground),transparent_85%)]',
    })
    ->add($loading ? [ // Loading states...
        '*:transition-opacity',
        $type === 'submit' ? '[&[disabled]>:not([data-laraliveui-loading-indicator])]:opacity-0' : '[&[data-loading]>:not([data-laraliveui-loading-indicator])]:opacity-0 [&[data-laraliveui-loading]>:not([data-laraliveui-loading-indicator])]:opacity-0',
        $type === 'submit' ? '[&[disabled]>[data-laraliveui-loading-indicator]]:opacity-100' : '[&[data-loading]>[data-laraliveui-loading-indicator]]:opacity-100 [&[data-laraliveui-loading]>[data-laraliveui-loading-indicator]]:opacity-100',
        $type === 'submit' ? '[&[disabled]]:pointer-events-none' : 'data-loading:pointer-events-none data-laraliveui-loading:pointer-events-none',
    ] : [])
    ...
    $attributes = $attributes->merge([
        'data-laraliveui-group-target' => ! in_array($variant, ['subtle', 'ghost']),
    ]);
@endphp

<laraliveui:with-tooltip :$attributes>
    <laraliveui:button-or-link-pure :$type :attributes="$attributes->class($classes)" data-laraliveui-button>
        <?php if ($loading): ?>
            <div class="absolute inset-0 flex items-center justify-center opacity-0" data-laraliveui-loading-indicator>
                <laraliveui:icon icon="loading" :variant="$iconVariant" :class="$iconClasses" />
            </div>
        <?php endif; ?>

        <?php if (is_string($iconLeading) && $iconLeading !== ''): ?>
            <laraliveui:icon :icon="$iconLeading" :variant="$iconVariant" :class="$iconClasses" />
        ...
        <?php if (is_string($iconTrailing) && $iconTrailing !== ''): ?>
            ...
            <laraliveui:icon :icon="$iconTrailing" :variant="$iconTrailingVariant" :class="$iconTrailingClasses" />
        <?php elseif ($iconTrailing): ?>
            {{ $iconTrailing }}
        <?php endif; ?>
    </laraliveui:button-or-link-pure>
</laraliveui:with-tooltip>
