...
<?php if ($as !== 'button') : ?><a {{ $attributes->class($classes) }} data-laraliveui-link <?php if ($external) : ?>target="_blank"<?php endif; ?>>{{ $slot }}</a><?php else : ?><button {{ $attributes->merge(['class' => $classes, 'type' => 'button']) }} data-laraliveui-link>{{ $slot }}</button><?php endif; ?>