...
<laraliveui:button-or-link :attributes="$attributes->class($classes)" data-laraliveui-navmenu-item>
    <?php if ($indent): ?>
        <div class="w-7"></div>
    <?php endif; ?>

    <?php if (is_string($icon) && $icon !== ''): ?>
        <laraliveui:icon :$icon :variant="$iconVariant" :class="$iconClasses" data-navmenu-icon />
    <?php elseif ($icon): ?>
        {{ $icon }}
    <?php endif; ?>
