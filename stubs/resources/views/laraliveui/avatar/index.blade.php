...
$badgeClasses = Laraliveui::classes()
...
<laraliveui:with-tooltip :$tooltip :$attributes>
    <laraliveui:button-or-link :attributes="$attributes->class($classes)->merge($circle ? ['data-circle' => 'true'] : [])" :$as :$href data-laraliveui-avatar data-slot="avatar" data-size="{{ $size }}">
        <?php if ($src): ?>
            <img src="{{ $src }}" alt="{{ $alt ?? $name }}" class="rounded-[var(--avatar-radius)] size-full object-cover">
        <?php elseif ($icon): ?>
            <laraliveui:icon :name="$icon" :variant="$iconVariant" :class="$iconClasses" />
