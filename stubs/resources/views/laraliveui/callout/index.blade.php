...
@php
    ...
    $iconWrapperClasses = Laraliveui::classes()
...
<div {{ $attributes->class($classes) }} data-laraliveui-callout>
    ...
    <div class="ps-2 flex-1 {{ $inline ? '@md:flex @md:[&>[data-slot="content"]:has([data-slot="heading"]):has([data-slot="text"])+[data-slot="actions"]]:p-2' : '' }}">
        <div class="flex-1 py-2 pe-3 @md:pe-4 flex flex-col justify-center gap-2" data-slot="content">
            <?php if ($heading): ?>
                <laraliveui:callout.heading>{{ $heading }}</laraliveui:callout.heading>
            <?php endif; ?>

            <?php if ($text): ?>
                <laraliveui:callout.text>{{ $text }}</laraliveui:callout.text>
            <?php endif; ?>
