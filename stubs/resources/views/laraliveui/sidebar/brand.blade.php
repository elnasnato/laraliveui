...
$classes = Laraliveui::classes()
    ->add('h-10 min-w-0 flex items-center px-2 in-data-laraliveui-sidebar-collapsed-desktop:w-10 in-data-laraliveui-sidebar-collapsed-desktop:px-0 in-data-laraliveui-sidebar-collapsed-desktop:justify-center')
    ->add('in-data-laraliveui-sidebar-collapsed-desktop:in-data-laraliveui-sidebar-active:absolute')
    ->add('in-data-laraliveui-sidebar-collapsed-desktop:in-data-laraliveui-sidebar-active:opacity-0')
    ;

$textClasses = Laraliveui::classes()
    ->add('min-w-0 text-sm font-medium truncate [:where(&)]:text-zinc-800 dark:[:where(&)]:text-zinc-100')
    ;
@endphp

<?php if ($name): ?>
    <a href="{{ $href }}" {{ $attributes->class([ $classes, 'gap-2' ]) }} data-laraliveui-sidebar-brand>
        ...
        <div class="{{ $textClasses }} in-data-laraliveui-sidebar-collapsed-desktop:hidden">{{ $name }}</div>
    </a>
<?php else: ?>
    <a href="{{ $href }}" {{ $attributes->class($classes) }} data-laraliveui-sidebar-brand>
        <?php if ($logo instanceof \Illuminate\View\ComponentSlot): ?>
            <div {{ $logo->attributes->class('flex items-center justify-center [:where(&)]:h-6 [:where(&)]:min-w-6 [:where(&)]:rounded-sm overflow-hidden shrink-0') }}>
                {{ $logo }}
            </div>
