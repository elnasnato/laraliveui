...
$inputAttributes = Laraliveui::attributesAfter('input:', $attributes, []);
...
$inputLoadingClasses = Laraliveui::classes()
...
<?php if ($type === 'file'): ?>
    <laraliveui:with-field :$attributes :$name>
        <laraliveui:input.file :$attributes :$name :$size />
    </laraliveui:with-field>
<?php elseif ($as !== 'button'): ?>
    <laraliveui:with-field :$attributes :$name>
        <div {{ $attributes->only('class')->class('w-full relative block group/input') }} data-laraliveui-input>
            ...
            <?php if ($loading || $countOfTrailingIcons > 0): ?>
                <div class="absolute top-0 bottom-0 flex items-center gap-x-1.5 pe-2 border-e border-transparent end-0 text-xs text-zinc-400">
                    ...
                    <?php if ($loading): ?>
                        <laraliveui:icon name="loading" :variant="$iconVariant" :class="$iconClasses" wire:loading :wire:target="$wireTarget" />
                    <?php endif; ?>

                    <?php if ($clearable): ?>
                        <laraliveui:input.clearable inset="left right" :$size :$iconVariant />
                    <?php endif; ?>
                    ...
                    <?php if ($expandable): ?>
                        <laraliveui:input.expandable inset="left right" :$size :$iconVariant />
                    <?php endif; ?>

                    <?php if ($copyable): ?>
                        <laraliveui:input.copyable inset="left right" :$size :$iconVariant />
                    <?php endif; ?>

                    <?php if ($viewable): ?>
                        <laraliveui:input.viewable inset="left right" :$size :$iconVariant />
                    <?php endif; ?>
                    ...
                    <?php if (is_string($iconTrailing) && $iconTrailing !== ''): ?>
                        ...
                        ?>
                        <laraliveui:icon :icon="$iconTrailing" :variant="$iconVariant" :class="$trailingIconClasses" />
