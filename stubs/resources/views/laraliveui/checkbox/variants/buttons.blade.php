...
$iconAttributes = Laraliveui::attributesAfter('icon:', $attributes, [
...
<ui-checkbox {{ $attributes->class($classes) }} data-laraliveui-control data-laraliveui-checkbox-buttons tabindex="-1" data-laraliveui-field>
    <?php if (is_string($icon) && $icon !== ''): ?>
        <laraliveui:icon :icon="$icon" :attributes="$iconAttributes" />
    <?php elseif ($icon): ?>
        {{ $icon }}
    <?php endif; ?>
