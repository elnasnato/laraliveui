...
$classes = Laraliveui::classes()
    ...
    ->add(match ($accent) {
        true => [
            '[&:hover_[data-laraliveui-checkbox-indicator]]:border-[var(--haze-border)] dark:[&:hover_[data-laraliveui-checkbox-indicator]]:border-white/10',
...
{{-- We have to put "data-laraliveui-field" so that a single box can be disabled without "disabling" the group field label... --}}
<ui-checkbox {{ $attributes->class($classes) }} data-laraliveui-control data-laraliveui-checkbox-cards tabindex="-1" data-laraliveui-field>
    <?php if ($label): ?>
        <div class="flex-1 flex gap-2">
            ...
            <div class="flex-1">
                <laraliveui:heading>{{ $slot->isNotEmpty() ? $slot : $label }}</laraliveui:heading>

                <?php if ($description): ?>
                    <laraliveui:subheading size="sm">{{ $description }}</laraliveui:subheading>
                <?php endif; ?>
            </div>
        </div>
