...
$iconClasses = Laraliveui::classes($square ? 'size-6' : 'size-5');
...
<laraliveui:button-or-link :attributes="$attributes->class($classes)" data-laraliveui-navbar-items>
    <?php if ($icon): ?>
        <div class="relative">
    ...
    <?php elseif (is_string($iconTrailing) && $iconTrailing !== ''): ?>
        <laraliveui:icon :icon="$iconTrailing" :variant="$iconVariant" class="{!! $iconClasses !!} ms-1" />
    <?php elseif ($iconTrailing): ?>
        {{ $iconTrailing }}
    <?php endif; ?>

    <?php if (isset($badge) && $badge !== ''): ?>
        <?php $badgeAttributes = Laraliveui::attributesAfter('badge:', $attributes, ['color' => $badgeColor, 'class' => 'ms-2']); ?>
        <laraliveui:navbar.badge :attributes="$badgeAttributes">{{ $badge }}</laraliveui:navbar.badge>
    <?php endif; ?>
