...
extract(Laraliveui::forwardedAttributes($attributes, ['scrollTo']));
?>
...
@if ($simple)
    <div {{ $attributes->class('pt-3 border-t border-zinc-100 dark:border-zinc-700 flex justify-between items-center') }} data-laraliveui-pagination>
        <div></div>
            ...
            <div class="flex items-center bg-white border border-zinc-200 rounded-[8px] p-[1px] dark:bg-white/10 dark:border-white/10">
                @if ($paginator->onFirstPage())
                    <div class="flex justify-center items-center size-8 sm:size-6 rounded-[6px] text-zinc-300 dark:text-zinc-500">
                        <laraliveui:icon.chevron-left variant="micro" class="rtl:hidden" />
                        <laraliveui:icon.chevron-right variant="micro" class="hidden rtl:inline" />
                    </div>
                @else
                ...
                @if ($paginator->hasMorePages())
                    @if(method_exists($paginator,'getCursorName'))
                        <button type="button" wire:key="cursor-{{ $paginator->getCursorName() }}-{{ $paginator->nextCursor()->encode() }}" wire:click="setPage('{{$paginator->nextCursor()->encode()}}','{{ $paginator->getCursorName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" class="flex justify-center items-center size-8 sm:size-6 rounded-[6px] text-zinc-400 dark:text-white hover:bg-zinc-100 dark:hover:bg-white/20 hover:text-zinc-800 dark:hover:text-white">
                            <laraliveui:icon.chevron-right variant="micro" class="rtl:hidden" />
                            <laraliveui:icon.chevron-left variant="micro" class="hidden rtl:inline" />
                        </button>
                    @else
...
@else
    <div {{ $attributes->class('@container pt-3 border-t border-zinc-100 dark:border-zinc-700 flex justify-between items-center gap-3') }} data-laraliveui-pagination>
